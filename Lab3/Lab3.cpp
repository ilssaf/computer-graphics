...
WCHAR szTitle[MAX_LOADSTRING] = L"Khamidullin Ilsaf";
WCHAR szWindowClass[MAX_LOADSTRING] = L"Lab3";
...
HRESULT InitializeGraphics()
{
    ...
    bd.ByteWidth = sizeof(Vertex) * ARRAYSIZE(g_CubeVertices);
...
void RenderFrame()
{
    ...
    float radius = 3.0f;
    float camX = sinf(g_CameraAngle) * radius;
    float camZ = cosf(g_CameraAngle) * radius;
    XMVECTOR eyePos = XMVectorSet(camX, 1.5f, camZ, 0.0f);
    ...
    g_pImmediateContext->UpdateSubresource(g_pModelBuffer, 0, nullptr, &mT, 0, 0);

    D3D11_MAPPED_SUBRESOURCE mappedResource;
    HRESULT hr = g_pImmediateContext->Map(g_pVPBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    if (SUCCEEDED(hr))
    {
